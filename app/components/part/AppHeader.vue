...
<template>
  <header
    class="sticky top-0 z-50 bg-linear-to-r from-[#fe8f47] from-10% via-[#ca390f] via-20% to-[#8b1504] to-100% w-full border-b border-zinc-200/50 backdrop-blur-md transition-all duration-500"
    :class="{
      'h-screen overflow-y-auto from-[#8b1504]! via-80% to-[#fe8f47]':
        isMenuOpen,
    }">
    <div
      class="lg:container lg:mx-auto h-20 flex items-center justify-between shrink-0"
      ...
      <ClientOnly
        ><NuxtLink
          ...
          <PartLogo
            class="max-w-44 lg:max-w-48 after:content-[''] after:absolute after:right-0 after:-top-4 after:-z-10 after:h-50 after:w-50 after:bg-[url('~/assets/webp/shape.webp')] after:bg-contain after:bg-no-repeat after:rotate-60 after:-scale-x-100 after:pointer-events-none before:content-[''] before:absolute before:left-2/3 before:top-1/10 before:z-30 before:h-22 before:w-40 before:bg-[url('~/assets/webp/leaf-5.webp')] before:bg-contain before:bg-no-repeat before:pointer-events-none before:filter-[drop-shadow(-10px_3px_5px_rgba(0,0,0,0.5))]" /> </NuxtLink
      ></ClientOnly>

      <nav
        class="mx-auto pt-8 lg:block hidden absolute lg:static inset-0 max-lg:container"
        :class="{ 'block! mt-30 pt-0 ': isMenuOpen }">
        <ul
